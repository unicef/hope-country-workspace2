...
def _collect_by_prefix(data: dict[str, Any], prefix: str) -> dict[str, dict[str, Any]]:
    ...
    Args:
        ...
        prefix (str): The prefix to filter and group data.

    Returns:
        dict[str, dict[str, Any]]: A dictionary where keys are indices extracted from prefixed keys,
        and values are dictionaries containing the corresponding grouped fields.
    ...
    for k, v in data.items():
        if (stripped := k.removeprefix(prefix)) != k:
            ...
            result.setdefault(index, {})[clean_field_name(field)] = v
    if not result:
